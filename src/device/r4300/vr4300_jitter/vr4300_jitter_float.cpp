...
bool vr4300_jitter_check_output_float(uint32_t* fcr31, int32_t value, int32_t *res)
{
    ...
    case FP_SUBNORMAL:
        ...
        fvalue = vr4300_jitter_fpu_flush_result(fvalue, *fcr31);
        memcpy(res, &fvalue, sizeof(*res));
...
void VR4300_Jitter::compile_fpu_check_output_float(struct jit_instr *op)
{
    ...
    {
        RCX64Reg Rd = m_fpr.Bind32(USE32_D(op->d), RCMode::Write);
        RegCache::Realize(Rd);
...
void VR4300_Jitter::recompile_CVT_S_W(struct jit_instr *op)
{
...

    {
        ...
        load_cop1_register_to_host_register_s(op, 32, op->s, gprscratch);
...
void VR4300_Jitter::recompile_CEIL_W_S(struct jit_instr *op)
{
...

    {
        ...
        CVTTSS2SI(gprscratch, R(XMM0));
        MOVD_xmm(Rd, R(gprscratch));
    }
...
template <typename ftype, typename vtype>
bool vr4300_jitter_C_cond_fmt_checks(uint32_t* fcr31, vtype value, vtype value2, bool cf, bool quiet)
{
    ftype fvalue, fvalue2;
    memcpy(&fvalue, &value, sizeof(value));
    memcpy(&fvalue2, &value2, sizeof(value2));
    if (isnan(fvalue) && (!quiet || qnan(fvalue))) {
        (*fcr31) |= FCR31_CAUSE_INVALIDOP_BIT;
        if (!((*fcr31) & FCR31_ENABLE_INVALIDOP_BIT)) {
            (*fcr31) |= FCR31_FLAG_INVALIDOP_BIT;
        } else {
    ...
    if (isnan(fvalue2) && (!quiet || qnan(fvalue2))) {
        (*fcr31) |= FCR31_CAUSE_INVALIDOP_BIT;
        if (!((*fcr31) & FCR31_ENABLE_INVALIDOP_BIT)) {
            (*fcr31) |= FCR31_FLAG_INVALIDOP_BIT;
        } else {
...
bool vr4300_jitter_C_cond_fmt_checks_double(uint32_t* fcr31, int64_t value, int64_t value2, bool cf, bool quiet)
{
    return vr4300_jitter_C_cond_fmt_checks<double, int64_t>(fcr31, value, value2, cf, quiet);
}

bool vr4300_jitter_C_cond_fmt_checks_float(uint32_t* fcr31, int32_t value, int32_t value2, bool cf, bool quiet)
{
    return vr4300_jitter_C_cond_fmt_checks<float, int32_t>(fcr31, value, value2, cf, quiet);
}

void VR4300_Jitter::compile_C_cond_fmt_checks(struct jit_instr *op, bool cf, bool is_double, bool quiet)
{
#ifdef ACCURATE_FPU_BEHAVIOR
    // TODO: asm impl
    BitSet32 registers_in_use = caller_saved_registers_in_use();
    ...
    if (is_double) {
        ...
        MOVD_xmm(R(ABI_PARAM2), XMM0);
        MOVD_xmm(R(ABI_PARAM3), XMM1);
    }
    MOV(32, R(ABI_PARAM4), Imm32(cf));
    MOV(32, R(ABI_PARAM5), Imm32(quiet));
    if (is_double) {
        ABI_CallFunction(vr4300_jitter_C_cond_fmt_checks_double);
    } else {
        ABI_CallFunction(vr4300_jitter_C_cond_fmt_checks_float);
    }
    ABI_PopRegistersAndAdjustStack(registers_in_use, 0);
...
void VR4300_Jitter::recompile_C_cond_fmt(struct jit_instr *op, int fmt)
{
    ...
    switch(op->c) {
        case 0: {
            ...
            // at least one is nan
            compile_C_cond_fmt_checks(op, false, is_double, true);
        ...
        case 1: {
            ...
            // at least one is nan
            compile_C_cond_fmt_checks(op, true, is_double, true);
        ...
        case 8: {
            ...
            // at least one is nan
            compile_C_cond_fmt_checks(op, false, is_double, false);
        ...
        case 9: {
            ...
            // at least one is nan
            compile_C_cond_fmt_checks(op, true, is_double, false);
...
void VR4300_Jitter::recompile_MUL_S(struct jit_instr *op)
{
    ...
    {
        RCOpArg Rt = m_fpr.Use32(USE32_TF(op->t), RCMode::Read);
        RCOpArg Rs = m_fpr.Use32(USE32_S(op->s), RCMode::Read);
...
void VR4300_Jitter::store_host_register_to_cop1_register(struct jit_instr *op, int bits, const RCOpArg &cpu_val, int reg, bool flush_upper, bool integer_usage)
{
    if (bits == 32) {
        RCX64Reg Rt = m_fpr.Bind32(integer_usage ? USE32_TI(reg) : USE32_TF(reg), RCMode::Write, flush_upper);
        RegCache::Realize(Rt);
...
void VR4300_Jitter::load_cop1_register_to_host_register(struct jit_instr *op, int bits, int reg, const RCX64Reg &target, bool integer_usage)
{
    if (bits == 32) {
        RCOpArg Rt = m_fpr.Use32(integer_usage ? USE32_TI(reg) : USE32_TF(reg), RCMode::Read);
        RegCache::Realize(Rt);
...
void VR4300_Jitter::load_cop1_register_to_host_register_s(struct jit_instr *op, int bits, int reg, const RCX64Reg &target)
{
    if (bits == 32) {
        RCOpArg Rt = m_fpr.Use32(USE32_S(reg), RCMode::Read);
        RegCache::Realize(Rt);
...
void VR4300_Jitter::recompile_SWC1(struct jit_instr *op)
{
    ...
    if (!HOT_STATE->rdram_generate_slowcode && (!op->b || (m_gpr.IsImm(op->b) && vr4300_jitter_is_rdram_address(address)))) {
        ...
        load_cop1_register_to_host_register(op, 32, op->t, out_reg, true);
...
void VR4300_Jitter::recompile_LWC1(struct jit_instr *op)
{
    ...
    // LWC1 does not change the upper bits
    store_host_register_to_cop1_register(op, 32, RCOpArg::R(ABI_RETURN), op->t, false, true);
}
...
{
    VALIDATE_IN(op, t);
    VALIDATE_FOUT32TI(op, s);

    // MTC1 does not change the upper bits
    if (op->t) {
        RCX64Reg Rd = m_fpr.Bind32(USE32_TI(op->s), RCMode::Write, false);
...
void VR4300_Jitter::recompile_MFC1(struct jit_instr *op)
{
    ...
    if (op->t) {
        ...
        // Note not using _s here for compat.
        load_cop1_register_to_host_register(op, 32, op->s, Rt, true);
    }
}
...
void VR4300_Jitter::recompile_UNIMPLEMENTED_COP1(struct jit_instr *op)
{
    compile_fpu_reset_cause(op);
