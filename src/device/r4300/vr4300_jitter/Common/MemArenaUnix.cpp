...
{
  const std::string file_name = std::string("/") + std::string{base_name} + "." + std::to_string(getpid());
  m_shm_fd = shm_open(file_name.c_str(), O_RDWR | O_CREAT | O_EXCL, 0600);
  if (m_shm_fd == -1)
  {
    ERROR_LOG_FMT(MEMMAP, "shm_open failed: %s", strerror(errno));
    return;
  }
...
void* MemArena::CreateView(s64 offset, size_t size)
{
  ...
  {
    NOTICE_LOG_FMT(MEMMAP, "mmap failed %s\n", strerror(errno));
    return nullptr;
  }
  else
  {
...
u8* MemArena::ReserveMemoryRegion(size_t memory_size)
{
  ...
  if (base == MAP_FAILED)
  {
    ASSERT_MSG(MEMMAP, 0, "Failed to map enough memory space: %s", LastStrerrorString().c_str());
    return nullptr;
  }
...
void* MemArena::MapInMemoryRegion(s64 offset, size_t size, void* base)
{
  ...
  if (retval == MAP_FAILED)
  {
    NOTICE_LOG_FMT(MEMMAP, "mmap failed (2) %s\n", strerror(errno));
    return nullptr;
  }
  else
  {
...
void* LazyMemoryRegion::Create(size_t size)
{
  ...
  {
    NOTICE_LOG_FMT(MEMMAP, "Memory allocation of %ld bytes failed.", size);
    return nullptr;
  }
