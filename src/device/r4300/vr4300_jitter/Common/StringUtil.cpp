...
static std::string UTF16ToCP(u32 code_page, std::wstring_view input)
{
                                  ...
                                  output.data(), static_cast<int>(output.size()), nullptr, nullptr))
  {
    ...
    ERROR_LOG_FMT(COMMON, "WideCharToMultiByte Error in String '%s': %d", WStringToUTF8(input).c_str(),
                  error_code);
    return {};
  }
...
std::string CodeTo(const char* tocode, const char* fromcode, std::basic_string_view<T> input)
{
  ...
  {
    ERROR_LOG_FMT(COMMON, "Iconv initialization failure [%s]: %s", fromcode, strerror(errno));
  }
  else
  {
    ...
    while (src_bytes != 0)
    {
      ...
      if ((size_t)-1 == iconv_result)
      {
        ...
        {
          ERROR_LOG_FMT(COMMON, "iconv failure [%s]: %s", fromcode, strerror(errno));
          break;
        }
      }
    }
