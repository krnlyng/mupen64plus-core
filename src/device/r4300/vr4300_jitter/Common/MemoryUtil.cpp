...
void* AllocateExecutableMemory(size_t size)
{
  ...
  if (ptr == nullptr)
    ASSERT_MSG(MEMMAP, 0, "Failed to allocate executable memory");
...
void JITPageWriteDisableExecuteEnable()
{
  ...
  if (JITPageWriteNestCounter() < 0)
    ASSERT_MSG(MEMMAP, 0, "JITPageWriteNestCounter() underflowed");
...
void* AllocateMemoryPages(size_t size)
{
  ...
  if (ptr == nullptr)
    ASSERT_MSG(MEMMAP, 0, "Failed to allocate raw memory");
...
void* AllocateAlignedMemory(size_t size, size_t alignment)
{
  ...
  if (ptr == nullptr)
    ASSERT_MSG(MEMMAP, 0, "Failed to allocate aligned memory");
...
bool FreeMemoryPages(void* ptr, size_t size)
{
  if (ptr)
  {
    ...
    {
      ASSERT_MSG(MEMMAP, 0, "FreeMemoryPages failed!\nVirtualFree: %s", GetLastErrorString().c_str());
      return false;
    }
#else
...
bool ReadProtectMemory(void* ptr, size_t size)
{
  ...
  {
    ASSERT_MSG(MEMMAP, 0, "ReadProtectMemory failed!\nVirtualProtect: %s", GetLastErrorString().c_str());
    return false;
  }
#else
...
bool WriteProtectMemory(void* ptr, size_t size, bool allowExecute)
{
  ...
  {
    ASSERT_MSG(MEMMAP, 0, "WriteProtectMemory failed!\nVirtualProtect: %s", GetLastErrorString().c_str());
    return false;
  }
...
bool UnWriteProtectMemory(void* ptr, size_t size, bool allowExecute)
{
  ...
  {
    ASSERT_MSG(MEMMAP, 0, "UnWriteProtectMemory failed!\nVirtualProtect: %s", GetLastErrorString());
    return false;
  }
#elif !(defined(_M_ARM_64) && defined(__APPLE__))
  // MacOS 11.2 on ARM does not allow for changing the access permissions of pages
