...
void CPUInfo::Detect()
{
  ...
  if (func_id_max >= 1)
  {
    ...
    std::ostringstream cpu_id_stream;
    cpu_id_stream << std::hex << std::setfill('0') << std::setw(sizeof(u32) * 2) << family << ":" << model << ":" << std::setw(0) << stepping;
    cpu_id = cpu_id_stream.str();
