...
private:
  ...
  bool soft_flags_locked = false;
  bool soft_flags_modified = false;
...
public:
  ...
  void SoftLockFlags() { ASSERT_MSG(DYNA_REC, !soft_flags_locked, "Flags (soft) locked twice!"); soft_flags_modified = false; soft_flags_locked = true; }
  void SoftUnlockFlags() { soft_flags_locked = false; }
  bool SoftFlagsWereModified() { return soft_flags_modified; }
...
#undef CALL
  ...
  void ROUNDSS(X64Reg regOp, const OpArg& arg, u8 rounding_mode);
  void ROUNDSD(X64Reg regOp, const OpArg& arg, u8 rounding_mode);
  ...
  void CVTSI2SD64(X64Reg dest, const OpArg& src);
