...
void XEmitter::CheckFlags()
{
  ...
  if (soft_flags_locked) {
    soft_flags_modified = true;
  }
}
                      ...
                      bool warn_64bit_offset) const
{
  ...
  if (scale == SCALE_RIP)  // Also, on 32-bit, just an immediate address
  {
    ...
    ASSERT_MSG(DYNA_REC,
               ...
               "WriteRest: op out of range (%lx uses %lx)", ripAddr, offset);
...
void XEmitter::JMP(const u8* addr, const Jump jump)
{
  ...
  if (jump == Jump::Short)
  {
    ...
    ASSERT_MSG(DYNA_REC, distance >= -0x80 && distance < 0x80,
               "Jump::Short target too far away (%ld, 0x%lx), needs Jump::Near", distance, distance);
    // 8 bits will do
    Write8(0xEB);
  ...
  else
  {
    ...
    ASSERT_MSG(DYNA_REC, distance >= -0x80000000LL && distance < 0x80000000LL,
               "Jump::Near target too far away (%ld, 0x%lx), needs indirect register", distance, distance);
    Write8(0xE9);
...
void XEmitter::J_CC(CCFlags conditionCode, const u8* addr)
{
  ...
  if (distance < -0x80 || distance >= 0x80)
  {
    ...
    ASSERT_MSG(DYNA_REC, distance >= -0x80000000LL && distance < 0x80000000LL,
               "Jump target too far away (%ld, 0x%lx), needs indirect register", distance, distance);
    Write8(0x0F);
    Write8(0x80 + conditionCode);
    Write32((u32)(s32)distance);
  }
...
void XEmitter::TZCNT(int bits, X64Reg dest, const OpArg& src)
{
  CheckFlags();
  if (!cpu_info.bBMI1)
    ASSERT_MSG(DYNA_REC, 0, "Trying to use BMI1 on a system that doesn't support it. Bad programmer.");
...
void XEmitter::LZCNT(int bits, X64Reg dest, const OpArg& src)
{
  CheckFlags();
  if (!cpu_info.bLZCNT)
    ASSERT_MSG(DYNA_REC, 0, "Trying to use LZCNT on a system that doesn't support it. Bad programmer.");
...
void XEmitter::MOV(int bits, const OpArg& a1, const OpArg& a2)
{
  ...
  if (a1.IsSimpleReg() && a2.IsSimpleReg() && a1.GetSimpleReg() == a2.GetSimpleReg()) {
    ERROR_LOG_FMT(DYNA_REC, "Redundant MOV @ %p - bug in JIT?", code);
  }
...
{
  if (!cpu_info.bAVX)
    ASSERT_MSG(DYNA_REC, 0, "Trying to use AVX on a system that doesn't support it. Bad programmer.");
...
{
  if (arg.IsImm())
    ASSERT_MSG(DYNA_REC, 0, "BMI1/2 instructions don't support immediate operands.");
  if (size != 32 && size != 64)
    ASSERT_MSG(DYNA_REC, 0, "BMI1/2 instructions only support 32-bit and 64-bit modes!");
  const int W = size == 64;
  WriteVEXOp(opPrefix, op, regOp1, regOp2, arg, W, extrabytes);
}
...
{
  if (!cpu_info.bBMI2)
    ASSERT_MSG(DYNA_REC, 0, "Trying to use BMI2 on a system that doesn't support it. Bad programmer.");
  WriteBMIOp(size, opPrefix, op, regOp1, regOp2, arg, extrabytes);
}
...
void XEmitter::CVTSD2SI64(X64Reg regOp, const OpArg& in_arg)
{
  Write8(0xF2);
  OpArg arg = in_arg;
  arg.operandReg = regOp;
...
void XEmitter::CVTSS2SI64(X64Reg regOp, const OpArg& in_arg)
{
  Write8(0xF3);
  OpArg arg = in_arg;
  arg.operandReg = regOp;
...
void XEmitter::CVTSI2SD64(X64Reg regOp, const OpArg& in_arg)
{
  Write8(0xF2);
  OpArg arg = in_arg;
  arg.operandReg = regOp;
...
void XEmitter::CVTTSD2SI64(X64Reg regOp, const OpArg& in_arg)
{
  Write8(0xF2);
  OpArg arg = in_arg;
  arg.operandReg = regOp;
...
void XEmitter::CVTTSS2SI64(X64Reg regOp, const OpArg& in_arg)
{
  Write8(0xF3);
  OpArg arg = in_arg;
  arg.operandReg = regOp;
...
{
  if (reg > 7)
    ASSERT_MSG(DYNA_REC, 0, "The PSRAW-emitter does not support regs above 7");
  Write8(0x66);
  Write8(0x0f);
  Write8(0x71);
  Write8(0xE0 | reg);
...
{
  if (reg > 7)
    ASSERT_MSG(DYNA_REC, 0, "The PSRAD-emitter does not support regs above 7");
  Write8(0x66);
  Write8(0x0f);
  Write8(0x72);
  Write8(0xE0 | reg);
...
{
  if (!cpu_info.bSSSE3)
    ASSERT_MSG(DYNA_REC, 0, "Trying to use SSSE3 on a system that doesn't support it. Bad programmer.");
...
{
  if (!cpu_info.bSSE4_1)
    ASSERT_MSG(DYNA_REC, 0, "Trying to use SSE4.1 on a system that doesn't support it. Bad programmer.");
  WriteSSEOp(opPrefix, op, regOp, arg, extrabytes);
}
