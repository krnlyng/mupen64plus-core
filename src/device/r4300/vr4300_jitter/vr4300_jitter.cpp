...
void VR4300_Jitter::set_instruction_stats(struct jit_instr *instr)
{
    ...
    if (!instr->is_branch_or_jump) {
        ...
        if (!instr->has_a && !instr->has_x && !instr->has_c && instr->has_t && instr->has_s && !instr->has_d && !instr->has_k) {
            // MULT(U)...
...
void VR4300_Jitter::recompile_instruction(struct jit_instr *op)
{
    ...
    if (is_float && interpret_float && !op->is_branch_or_jump) {
        ...
        switch (op->operation) {
            ...
            case VR4300_OP_ROUND_W_W:
            case VR4300_OP_TRUNC_W_W:
            case VR4300_OP_CEIL_W_W:
            case VR4300_OP_FLOOR_W_W:
            case VR4300_OP_ROUND_L_L:
            case VR4300_OP_TRUNC_L_L:
            case VR4300_OP_CEIL_L_L:
            case VR4300_OP_FLOOR_L_L:
            case VR4300_OP_ROUND_W_L:
